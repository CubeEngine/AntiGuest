...
public class VehiclePrevention extends Prevention {
    ...
    public void enable() {
        ...
        if (getConfig().contains("checkAndPrevent.access")) {
            access = getConfig().getBoolean("checkAndPrevent.access");
            getConfig().set("checkAndPrevent.access", null);
        }
        if (getConfig().contains("checkAndPrevent.destruction")) {
            access = getConfig().getBoolean("checkAndPrevent.destruction");
            getConfig().set("checkAndPrevent.destruction", null);
        }
        if (getConfig().contains("checkAndPrevent.collision")) {
            access = getConfig().getBoolean("checkAndPrevent.collision");
            getConfig().set("checkAndPrevent.collision", null);
        }
        if (getConfig().contains("checkAndPrevent.creation")) {
            access = getConfig().getBoolean("checkAndPrevent.creation");
            getConfig().set("checkAndPrevent.creation", null);
        }
        access = getConfig().getBoolean("prevent.access");
        destruction = getConfig().getBoolean("prevent.destruction");
        collision = getConfig().getBoolean("prevent.collision");
        creation = getConfig().getBoolean("prevent.creation");
    }
